...
const prompt = ai.definePrompt({
  ...
  prompt: `You are a master Iraqi pharmacist assistant. Provide a dosage analysis in Arabic for the given medications based on the patient's age and notes. Your response MUST be a valid JSON object matching the output schema.
Patient Age: {{patientAge}}
{{#if patientNotes}}Notes: {{patientNotes}}{{/if}}
Medications:
{{#each medications}}
- {{tradeName}} ({{#if scientific_names}}{{#each scientific_names}}{{.}}{{#unless @last}}, {{/unless}}{{/each}}{{else}}N/A{{/if}})
{{/each}}
`,
});
...
//   input: { schema: DoseCalculationInputSchema },
//   output: { schema: DoseCalculationOutputSchema },
//   prompt: `You are an expert Iraqi pharmacist assistant. Your task is to provide a comprehensive analysis for a list of medications based on a patient's age and clinical notes. Some medications may be compound drugs with multiple scientific names.

// Patient's Age: {{patientAge}} years old.
// {{#if patientNotes}}
// Clinical Notes: {{patientNotes}}
// {{/if}}

// Medications to process:
// {{#each medications}}
// - **{{tradeName}}** ({{#if scientific_names}}Scientific Names: {{#each scientific_names}}{{.}}{{#unless @last}}, {{/unless}}{{/each}}{{/if}}) ({{dosage}} {{dosage_form}})
// {{/each}}
...
//     b.  **suggestedDose:** Calculate and suggest an appropriate dose, frequency, and duration based on the patient's age and clinical notes. For compound drugs, the dose should be appropriate for the combination of active ingredients.
...
//     d.  **warning:** Include any critical warnings or contraindications specific to this age group OR based on the provided clinical notes (e.g., "لا يستخدم لمرضى السكري", "يجب استشارة الطبيب للحامل").
